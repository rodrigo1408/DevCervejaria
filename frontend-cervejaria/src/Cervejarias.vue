   ...
   <a class="fixo button is-large is-danger is-loading" v-show="isLoading">Loading</a>
    ...
    <div class="columns">
      <div class="column is-12">
        ...
        <tbody>
          <tr class="brewery" v-for="brewery in breweries" :key="brewery">
            <td>{{brewery.name}}</td>
            <td>{{brewery.city}}</td>
            <td>{{brewery.phone}}</td>
            <!--<td class="is-icon">
    ...
    <section class="modal-card-body">
      ...
      <div class="column">
        <label class="label">Telefone</label>
           <p class="control">
                <input class="input" type="text" placeholder="(00) 000-00-00-00" v-model="selected.phone">
           </p>
      </div>
      ...
      <div class="column">
        <label class="label">Local</label>
           <p class="control">
        <input class="input" type="text" placeholder="Local" v-model="selected.city">
           </p>
      </div>
  ...
  methods: {
    ...
    removeBrewery(brewery) {
      ...
      swal(
        ...
        function() {
          self.$http.delete(`/breweries/${brewery.id}`).then(result => {
    ...
    saveBrewery() {
      if (this.selected.id != null) {
        //EDIT
        this.$http
          .put(`/breweries/${this.selected.id}`, this.selected)
          .then(
            response => {
              this.$set("selected", {});
          ...
          .finally(this.loadBreweries());
      } else {
        //NEW
        this.$http
          .post(`/breweries`, this.selected)
          .then(
            response => {
              this.$set("selected", {});
              this.$set("showModal", false);
